...
export const gear_experience: GearExperience[] = [{ stars: 6, exp_granted: 1200 }];
...
export const calculations = (
	current_level: ExpLevelRequirement,
	target_level: ExpLevelRequirement
): ICalculations => ({
	exp_required: target_level.exp_required - current_level.exp_required,
	gear_required: gear_experience.map((gear: GearExperience) =>
		Math.ceil(target_level.exp_required / gear.exp_granted)
	),
	charms_required: charms_experience.map((charm: CharmsExperience) =>
		Math.ceil(target_level.exp_required / charm.exp_granted)
	)
});
